...
export async function initiateTitleRenewal(userId) {
  return authFetch(
    '/api/transactions/title-renewal',
    { method: 'POST', headers: jsonHeaders, body: JSON.stringify({ userId }) },
...
// SHOP — owner CRUD
//
// The seller manages their own items. userId is checked against
// the token by ownsResource on the backend.
// ============================================================

export async function getMyShopItems(userId) {
  return authFetch(
    `/api/shop/${userId}/items`,
    {},
    'Failed to load shop items'
  );
}

export async function addShopItem(userId, { name, price, detail, hasImages }) {
  return authFetch(
    `/api/shop/${userId}/items`,
    {
      method: 'POST',
      headers: jsonHeaders,
      body: JSON.stringify({ name, price, detail, hasImages }),
    },
    'Failed to add item'
  );
}

export async function updateShopItem(userId, itemId, fields) {
  return authFetch(
    `/api/shop/${userId}/items/${itemId}`,
    {
      method: 'PATCH',
      headers: jsonHeaders,
...
export async function deleteShopItem(userId, itemId) {
  return authFetch(
    `/api/shop/${userId}/items/${itemId}`,
    { method: 'DELETE' },
...
export async function reorderShopItems(userId, order) {
  return authFetch(
    `/api/shop/${userId}/items/reorder`,
    {
      method: 'PATCH',
      headers: jsonHeaders,
...
// TITLES — public
//
// Titles are never sold. They are verified against the awarding
// authority, then issued. Nothing is charged until approved.
// ============================================================

export async function getTitlePrices() {
  return publicFetch('/api/titles/prices', {}, 'Title prices fetch failed');
}

export async function submitTitleRequest({ handleName, fullName, phone, email, title, awardYear, awardBody, document }) {
  const formData = new FormData();
  ...
  if (awardYear) formData.append('awardYear', awardYear);
  if (awardBody) formData.append('awardBody', awardBody);
...
export async function getTitleRequestStatus(id) {
  return publicFetch(`/api/titles/request/${id}`, {}, 'Request status fetch failed');
}
...
export async function getPublicProfile(handleName) {
  // Send the token if we have one — the BE releases the verified photo
  // only to logged-in members. Anonymous visitors still get the page.
  const token = getToken();
  const headers = token ? { Authorization: `Bearer ${token}` } : {};
  return publicFetch(`/api/profiles/public/${handleName}`, { headers }, 'Profile fetch failed');
}

// Number match check. The visitor already has a number — someone gave
// it to them. They ask one question about one handle: does this number
// belong to it? Yes or no comes back, nothing else. The number on file
// is never sent to the browser.
//
// Two attempts per handle per day. A 429 means the allowance is spent,
// a 404 means the handle has no number registered to check against.
export async function checkWhatsapp(handleName, number) {
  const token = getToken();
  const headers = token
  ...
  const res = await fetch(
    `${API_URL}/api/profiles/public/${handleName}/check-whatsapp`,
  ...
  // These are answers, not failures — the page renders a different
  // message for each, so they must not be thrown.
  if (res.status === 429) return { rateLimited: true, ...data };
  if (res.status === 404) return { notAvailable: true, ...data };
  if (res.status === 400) return { invalidNumber: true, ...data };
